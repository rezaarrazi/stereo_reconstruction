...
void ExperimentDesigner::CompareCameraPoseEstimation(std::size_t type, std::size_t keypoint_number, double distance_ratio, double ratio)
{
    ...
    for (std::size_t i = 0; i < image_pair_number; i++)
    {
        ...
        for (std::size_t j = 0; j < 4; j++)
        {
            ...
            // cv::Mat rotation = camera_pose_estimator_.GetRotation();
            // cv::Mat translation = camera_pose_estimator_.GetTranslation();

            // std::cout << rotation.type() << '\n';

            // std::cout << rotation << '\n';
            // std::cout << translation << '\n';

            // std::cout << rmses[j][0] << "   " << rmses[j][1] << "\n\n";
        }
